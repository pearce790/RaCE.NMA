...
#' @param J A numeric indicating the total number of treatments being compared.
...
#'
#' @export
sample_partition_normal <- function (ybar, J, nu, g, K, mu, sigma0, s, tau = tau, b_g = 0.5, d_g = 0.5){
  ...
  if (rbinom(1, 1, b_g) == 1) {
    ...
    if (all(S_g == 1)) {
      ...
      if (any((min(nu_new[c(k, K + 1)]) <= nu[-k]) & (nu[-k] <= max(nu_new[c(k, K + 1)])))) {
        ...
        logprob_accept <-
          ...
          sum(dnorm(nu_new[c(k, K+1)], mu, sigma0, log=T)) - dnorm(nu[k], mu, sigma0, log=T) +
    ...
    if (K == 1) {
      ...
      logprob_accept <-
        sum(unlist(lapply(1:J,function(j){dnorm(ybar[j], mean = nu_new[g_new[j]], sd = s[j],log=T)}))) -
        sum(unlist(lapply(1:J,function(j){dnorm(ybar[j], mean = nu[g[j]], sd = s[j],log=T)}))) +
        dnorm(nu_new[which_merge[1]], mu, sigma0, log=T) -
        sum(dnorm(nu[which_merge], mu, sigma0, log=T)) +
        logprior_partition[K - 1] - logprior_partition[K] + 
