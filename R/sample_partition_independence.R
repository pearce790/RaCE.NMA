...
#' @param s A vector of the estimated standard deviations of each intervention. The jth entry is the standard deviation of intervention j.
  ...
  if (rbinom(1, 1, b_g) == 1) {
    ...
    if (all(S_g == 1)) {
      ...
      if (any((min(nu_new[c(k, K + 1)]) <= nu[-k]) & (nu[-k] <= max(nu_new[c(k, K + 1)])))) {
        ...
        logprob_accept <-
          sum(unlist(lapply(1:J,function(j){dnorm(mu_hat[j], mean = nu_new[g_new[j]], sd = s[j],log=T)}))) -
          sum(unlist(lapply(1:J,function(j){dnorm(mu_hat[j], mean = nu[g[j]], sd = s[j],log=T)}))) +
