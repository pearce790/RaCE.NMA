...
#' @param credible The \code{credible} level for the interval estimate of MNBT. Defaults to 0.50.
...
#' @export
calculate_SUCRA_MNBT <- function(data=NULL,mcmc=NULL,credible=0.50,names=NULL){
  if(!is.null(data)){
    J <- ncol(data)
    if(is.null(names)){
      names <- paste(1:J)
    ...
    MNBT = apply(data_ranks,2,function(ranks){
      values <- quantile(ranks-1,c(0.5,(1-credible)/2,1-(1-credible)/2))
    ...
    names(result)[3] <- paste0("MNBT (",credible*100,"% CI)")
  }
  if(!is.null(mcmc)){
