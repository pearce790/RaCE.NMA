  ...
  for(J in c(6,12,18)){
    for(K in c(J/3,2*J/3,J)){
      for(s in c(0.1,0.3,0.5)){
        ...
        # For expediency, we set num_iters=50. To exactly reproduce values in the paper, set num_iters=5000
        mcmc <- mcmc_RCMVN(ybar=ybar,s=rep(s,J),mu=mean(ybar),sigma0=max(1,4*sd(ybar)),nu0=ybar,
                           tau=1,num_iters=50,nu_reps=2,chains=4,burn_prop=0.5,thin=3,suppressPrint = TRUE)
...
```{r}
# For expediency, we set num_iters=500. To exactly reproduce values in the paper, set num_iters=50000
mcmc <- mcmc_RCMVN(ybar=ybar,s=s,num_iters=500,nu_reps=2,chains=4,burn_prop=0.5,thin=1,suppressPrint = TRUE)
...
createtrace_omega(mcmc,names=treatments)+
  scale_x_continuous(breaks=seq(125000,250000,by=25000),
                     labels=paste0(c(125, 150, 175,200,225,250),"k"))
